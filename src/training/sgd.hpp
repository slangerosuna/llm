		...
		if (!llm::arch::sycl_ops::use_sycl_kernels()) {
			for (size_t pidx = 0; pidx < params.size(); ++pidx) {
				...
				for (size_t i = 0; i < p.values.size(); ++i) {
					...
					const float m = beta1_ * static_cast<float>(m_[pidx][i]) + (1.0f - beta1_) * grad;
					const float v = beta2_ * static_cast<float>(v_[pidx][i]) + (1.0f - beta2_) * grad * grad;
		...
		for (size_t pidx = 0; pidx < params.size(); ++pidx) {
			...
			q.submit([&](sycl::handler& h) {
				...
				h.parallel_for(sycl::range<1>(p.values.size()), [=](sycl::id<1> i) {
					...
					const float m = b1 * static_cast<float>(m_acc[i]) + (1.0f - b1) * grad;
					const float v = b2 * static_cast<float>(v_acc[i]) + (1.0f - b2) * grad * grad;
